...
# Column 0
# Connection label
conn_lbl = Label(window, text=base_conn_lbl, font=("Courier New", 14), padx=5, pady=5)
...
# MPH label
mph_lbl = Label(window, text=base_mph_lbl + "00 MPH", font=("Courier New", 64), padx=5, pady=5)
...
timing_adv_lbl = Label(window, text=base_timing_adv_lbl + "-- deg.", font=("Courier New", 30))
timing_adv_lbl.grid(column=1, row=4, sticky="nsew")

# Throttle position label
throttle_pos_lbl = Label(
    window, text=base_throttle_pos_lbl + "--%", font=("Courier New", 30)
)
...
def new_rpm(raw_rpm):
    ...
    if rpm_int >= 4600:
        ...
        shift_lbl.configure(text=active_shift_lbl, bg="red")
    if rpm_int < 4600:
        rpm_bar["style"] = "green.Horizontal.TProgressbar"
        shift_lbl.configure(text=base_shift_lbl, bg=rpm_lbl.cget("background"))
...
def new_coolant_temp(raw_coolant_temp):
    ...
    if int_coolant_temp < 85:
        coolant_temp_lbl.configure(bg="yellow")
    elif (int_coolant_temp >= 85) and (int_coolant_temp < 100):
        coolant_temp_lbl.configure(bg="green")
    elif (int_coolant_temp >= 100):
        coolant_temp_lbl.configure(bg="red")
